...
int main() {
    ...
    // Check backward diagonal (above major forward diagonal)
    for (int col = 1; col < SIZE - digits + 1; col++) {
        // Get initial product for diagonal
        product = 1;
        int row = 0;
...
int main() {
    ...
    for (int row = 0; row < 17; row++) {
        for (int col = 0; col < 20; col++) {
            long int a = getInt(grid, row*60 + col*3);
            long int b = getInt(grid, (row+1)*60 + col*3);
            long int c = getInt(grid, (row+2)*60 + col*3);
            long int d = getInt(grid, (row+3)*60 + col*3);
            product = a*b*c*d;
    ...
    for (int col = 0; col < 17; col++) {
        for (int row = 0; row < 20; row++) {
            long int a = getInt(grid, row*60 + col*3);
            long int b = getInt(grid, row*60 + (col+1)*3);
            long int c = getInt(grid, row*60 + (col+2)*3);
            long int d = getInt(grid, row*60 + (col+3)*3);
            product = a*b*c*d;
    ...
    for (int row = 0; row < 17; row++) {
        for (int col = 0; col < 17; col++) {
            long int a = getInt(grid, row*60 + col*3);
            long int b = getInt(grid, (row+1)*60 + (col+1)*3);
            long int c = getInt(grid, (row+2)*60 + (col+2)*3);
            long int d = getInt(grid, (row+3)*60 + (col+3)*3);
            product = a*b*c*d;
